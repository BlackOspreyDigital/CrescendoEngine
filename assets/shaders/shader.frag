...
void main() {
    ...
    vec3 F0_base = mix(vec3(1.0), tintColor, specularTint) * specularAmount * 0.08; 
    vec3 F0 = mix(F0_base, albedo, metallic);

    // --- HEMISPHERE GI (with AO!) ---
    float skyWeight = N.z * 0.5 + 0.5;
    
    // Grab ambient slider from the UI
    float ambientIntensity = global.params.x;

    // Multiply the base hemisphere light by the slider 
    vec3 hemiAmbient = mix(global.groundColor.rgb, global.skyColor.rgb, skyWeight) * albedo * 0.15 * ao * ambientIntensity;
    ...
    if (length(global.sunDirection.xyz) > 0.01) {
        ...
        float NDF;
        if (anisotropic > 0.01) {
            // Requires T (Tangent) and B (Bitangent) which you already extract for Normal Mapping!
            NDF = DistributionGGXAnisotropic(N, H, T, B, roughness, anisotropic);
        } else {
        ...
        vec3 finalDiffuse = PrincipleDiffuse(NdotL, NdotV, LdotH, roughness, subsurface, albedo);
        ...
        vec3 baseLight = (kD * finalDiffuse + baseSpecular);
    ...
    for(int i = 0; i < pLightCount; i++) {
        ...
        float LdotH_pt = max(dot(L_pt, H_pt), 0.0);
        vec3 finalDiffuse_pt = PrincipleDiffuse(NdotL_pt, NdotV_pt, LdotH_pt, roughness, subsurface, albedo);
        ...
        // --- BLENDING ---
        vec3 baseLight_pt = (kD_pt * finalDiffuse_pt + baseSpecular_pt);
    ...
    // Dim the HDR skybox reflection based on the UI slider 
    vec3 finalReflection = skyRefl * F_ambient * ambientIntensity;
